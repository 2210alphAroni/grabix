...
function buildArgs(url, formatId, outputTemplate) {
  ...
  if (formatId === "audio-flac")
    return [...base, "-f", "bestaudio/best", "-x", "--audio-format", "flac"];
  return [...base, "-f", "best"];
}
...
router.post("/", async (req, res) => {
  ...
  try {
    await ytDlp.execPromise(
      buildArgs(url.trim(), format_id || "video-1080", outTemplate),
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${asciiName}"; filename*=UTF-8''${encodeURIComponent(dlFilename)}`,
    );
    ...
    if (!res.headersSent)
      res
        .status(500)
        .json({ error: err.message?.slice(0, 300) || "Download failed" });
  }
});
