...
const API = "https://grabix-backend.onrender.com";
...
const PLATFORMS = {
  ...
  "facebook.com": {
    ...
    color: "#1877f2",
    isVideo: () =>
      /\/videos\/|\/reel\/|\/watch/.test(location.pathname) ||
  ...
  "instagram.com": {
    ...
    color: "#dd2a7b",
    isVideo: () =>
      /\/reels?\/|\/p\//.test(location.pathname) ||
      !!document.querySelector("video"),
  },
  "tiktok.com": {
    name: "TikTok",
    icon: "🎵",
    color: "#010101",
    isVideo: () => /\/@.+\/video\//.test(location.pathname),
  },
  "twitter.com": {
    name: "Twitter/X",
    icon: "𝕏",
    color: "#000000",
    isVideo: () =>
      /\/status\//.test(location.pathname) && !!document.querySelector("video"),
  },
  "x.com": {
    name: "Twitter/X",
    icon: "𝕏",
  ...
  "vimeo.com": {
    ...
    icon: "🎬",
    color: "#1ab7ea",
    isVideo: () => /^\/\d+/.test(location.pathname),
  },
  "twitch.tv": {
    name: "Twitch",
    icon: "🎮",
    color: "#6441a5",
    isVideo: () =>
      /\/videos\/|\/clip\//.test(location.pathname) ||
      !!document.querySelector("video"),
  },
  "reddit.com": {
    name: "Reddit",
    icon: "👽",
    color: "#ff4500",
    isVideo: () => !!document.querySelector("video,shreddit-player"),
  },
  "dailymotion.com": {
    name: "Dailymotion",
    icon: "📺",
...
function renderInfo(body, data) {
  ...
  body.querySelectorAll(".gp-tab").forEach((t) => {
    t.onclick = () => {
      body
        .querySelectorAll(".gp-tab")
        .forEach((x) => x.classList.remove("active"));
      t.classList.add("active");
      activeTab = t.dataset.t;
      document.getElementById("gp-fmts").innerHTML = fmtHTML(
        activeTab === "audio" ? aFmts : vFmts,
      );
      bindFmts(data);
    };
  });
...
async function doDownload(url, fmtId) {
  ...
  if (fill) {
    fill.className = "gp-fill indeterminate";
  }
